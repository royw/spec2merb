...
class XbmcInfo
  ...
  def to_xml
    ...
    begin
      ...
      Merb.logger.error { "Error creating nfo file - " + e.to_s}
      raise e
    end
    xml
  end
  ...
  def save
    begin
      if dirty?
        xml = self.to_xml
        unless xml.blank?
          Merb.logger.info { "updated #{@nfo_filespec}"}
          DvdProfiler2Xbmc.save_to_file(@nfo_filespec, xml)
        end
      end
    rescue Exception => e
      Merb.logger.error { "Unable to save xbmc info to #{@nfo_filespec} - #{e.to_s}" }
    end
  end
  ...
  def load
    begin
      ...
      Merb.logger.error { "Error loading \"#{@nfo_filespec}\" - " + e.to_s + "\n" + e.backtrace.join("\n") }
      raise e
    end
  end
